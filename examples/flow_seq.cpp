...
/*
  Copyright 2013, 2014, 2015, 2018 SINTEF Digital, Mathematics and cybernetics.
...
namespace Opm
{
    ...
    // ----------------- Main program -----------------
    int flowMain(int argc, char** argv)
    {
...
int main(int argc, char** argv)
{
    ...
    try {
        ...
        auto deck = std::make_shared< Opm::Deck >( parser.parseFile(deckFilename , parseContext) );
        if ( outputCout ) {
        ...
        // Blackoil case
        if( phases.size() == 3 ) {
            Opm::setDeck(*deck, *eclipseState, *schedule, *summary_config);
            return Opm::flowMain(argc, argv);
        }
        else
        {
            if (outputCout)
