...
namespace Details
{
    ...
    Matrix extractMatrix(const Matrix& m, const std::vector<int>& indices)
    {
        ...
        for (auto row = m.begin(); row != m.end(); ++row) {
            if (row.index() == indices[newrow]) {
                index_map[row.index()] = newrow;
                ++newrow;
                if (newrow == n) {
                    break;
                }
        ...
        for (auto row = res.createbegin(); row != res.createend(); ++row) {
            ...
            while (from_row.index() < indices[row.index()]) {
                ++from_row;
            }
            assert(from_row.index() == indices[row.index()]);
    ...
    bool matrixEqual(const Matrix& m1, const Matrix& m2)
    {
        ...
        if (m1.M() != m1.M()) return false;
        if (m1.nonzeroes() != m1.nonzeroes()) return false;
