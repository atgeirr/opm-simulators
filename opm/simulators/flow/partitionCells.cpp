...
namespace {
                     ...
                     const Opm::ZoltanPartitioningControl<Element>& zoltan_ctrl)
    {
        auto g2l = std::unordered_map<int,int>{};
        {
            ...
            for (const auto& cell : elements(grid_view, Dune::Partitions::interior)) {
                const auto globIx = zoltan_ctrl.local_to_global(zoltan_ctrl.index(cell));
                g2l.insert_or_assign(globIx, localCell++);
            }
        }

        const auto myPartitionInput = groupInputByProcessID(rawInput, grid_view.comm());

        auto partition = std::vector<int>(g2l.size(), -1);
        for (auto i = 0*myPartitionInput.size(); i < myPartitionInput.size(); i += 2) {
            auto cellPos = g2l.find(myPartitionInput[i + 0]);
            if (cellPos != g2l.end()) {
                partition[cellPos->second] = myPartitionInput[i + 1];
            }
        }
...
namespace {
                              ...
                              const Opm::ZoltanPartitioningControl<Element>& zoltan_ctrl)
    {
        ...
        return Opm::util::compressAndCountPartitionIDs
            (scatterPartition(input, grid_view, zoltan_ctrl));
    }
} // Anonymous namespace
