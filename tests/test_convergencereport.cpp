...
BOOST_AUTO_TEST_CASE(Failures)
{
    ...
    s1.setReservoirFailed({CR::ReservoirFailure::Type::Cnv, CR::Severity::Normal, 2, 1234, 0.1234});
    {
        ...
        BOOST_CHECK(f.cellNumber() == 1234);
        BOOST_CHECK(f.magnitude() == 0.1234);
        BOOST_CHECK(s1.wellFailures().empty());
        BOOST_CHECK(s1.severityOfWorstFailure() == CR::Severity::Normal);
    }

    Opm::ConvergenceReport s2;
    s2.setWellFailed({CR::WellFailure::Type::ControlTHP, CR::Severity::Normal, -1, "PRODUCER-123", 0.1234});
    s2.setWellFailed({CR::WellFailure::Type::MassBalance, CR::Severity::TooLarge, 2, "INJECTOR-XYZ", 0.2345});
    {
        ...
        BOOST_CHECK(f0.magnitude() == 0.1234);
        ...
        BOOST_CHECK(f1.magnitude() == 0.2345);
